...
def plot_confusion_matrix(y_test:np.ndarray, y_pred:np.ndarray, features:np.ndarray, c_name:str) -> None:
    plt.imshow(confusion_matrix(y_test, y_pred))
    plt.xticks(np.arange(len(features)), features, rotation=45)
    plt.yticks(np.arange(len(features)), features)
    plt.ylabel("Véritable classe associée")
    plt.xlabel("Classe prédite")
    plt.title("Matrice de confusion pour {}".format(c_name))
    plt.colorbar()
    plt.show()
...
if __name__ == '__main__':
    ...
    print("Taux de reconnaissance de KPPV  : {}/{}".format(knn_good_predictions, n_test_items))
    knn_score = knn_good_predictions / n_test_items
    print("Score de précision associé : {}".format(knn_score))
    plot_confusion_matrix(y_test, knn_predictions, cons_types, "KPPV")
